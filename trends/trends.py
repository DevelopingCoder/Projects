...
def extract_words(text):
    ...
    def join(letters):
        #takes a list of strings and joins them together
        if letters == []:
            return ""
...
def analyze_tweet_sentiment(tweet):
    ...
    for w in words:
        value=get_word_sentiment(w)
        if has_sentiment(value):
            sentiment_sum += sentiment_value(value)
            sentiment_words += 1
    if sentiment_words==0:
        return make_sentiment(None)
    average = sentiment_sum/sentiment_words
...
def find_centroid(polygon):
    ...
    Area = (calculate_summations(polygon, 1) / 2)
    if Area == 0:
        centroid_x = latitude(polygon[0])
        centroid_y = longitude(polygon[0])
    else:
        centroid_x = (calculate_summations(polygon, 2) / (6 * Area))
        centroid_y = (calculate_summations(polygon, 3) / (6 * Area))        
...
def average_sentiments(tweets_by_state):
    ...
    """
    current_dict = {}
    sentimentless_tweets = {}
    for state in tweets_by_state: #goes through every state in the dictionary and passes it through the loop
        for tweet in tweets_by_state[state]: #Analyzes all the tweets of the state
            if has_sentiment(analyze_tweet_sentiment(tweet)):
                if state in current_dict:
                    current_dict[state] = current_dict[state] + sentiment_value(analyze_tweet_sentiment(tweet))
                else:
                    current_dict[state] = sentiment_value(analyze_tweet_sentiment(tweet))
            else:
                if state in sentimentless_tweets:
                    sentimentless_tweets[state] += 1
                else:
                    sentimentless_tweets[state] = 1
        if state not in sentimentless_tweets:
            sentimentless_tweets[state] = 0
    for state in current_dict: #only choosing states w/ sentiment values
        current_dict[state] = current_dict[state] / (len(tweets_by_state[state]) - sentimentless_tweets[state])
    return current_dict
