...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        newframe = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, newframe)
    elif isinstance(procedure, MuProcedure):
        newframe = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, newframe)
    else:
    ...
    try:
        def convert(args):
            if args==nil:
                ...
                return [args.first] + convert(args.second)
        pylist = convert(args)
...
def do_mu_form(vals):
    ...
    #make tests with begin. similar to lambda
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        binding = scheme_eval(vals.second.first, env)
        env.define(target, binding)
        return target
        ...
        if not scheme_symbolp(target.first):
            raise SchemeError
        if vals.second.second!=nil:
            env.define(target.first, LambdaProcedure(formals, Pair('begin', vals.second) , env))
        else:
            env.define(target.first, LambdaProcedure(formals,vals.second.first, env))
        return target.first
    else:
...
def do_let_form(vals, env):
    ...
    def name_helper(binding):
        check_form(binding, 2, 2)
        if scheme_symbolp(binding.first):
            return binding.first
        raise SchemeError
    names = bindings.map(name_helper)
    values = bindings.map(lambda binding: scheme_eval(binding.second.first, env))
...
def do_if_form(vals, env):
    ...
    #implement evaluating functions within the if statement
...
def do_and_form(vals, env):
    ...
    if vals.second == nil:
        return vals.first
    first, rest = scheme_eval(vals.first, env), vals.second
    if scheme_pairp(vals.first) and vals.first.first == "quote": #Takes care of the quote form
        if scheme_false(vals.first.second.first):
            return False
        first = vals.first
...
def do_or_form(vals, env):
    ...
    if vals.second == nil:
        return vals.first
    first, rest = scheme_eval(vals.first, env), vals.second
    if scheme_pairp(vals.first) and vals.first.first == "quote":
        if scheme_true(vals.first.second.first):
            return vals.first
        first = False  #solves the case (or '#f 5). which should return 5
...
def do_begin_form(vals, env): #add tests to check if we need to scheme eval
...
def check_formals(formals):
    ...
    if formals == nil:
        ...
        for f in formals.second:
            if f == formals.first:
                raise SchemeError
        return check_formals(formals.second)
    raise SchemeError
