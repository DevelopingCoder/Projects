...
(define (zip pairs)
  (list (apply-to-all (lambda (pairs) (car pairs)) pairs)
    (apply-to-all (lambda (pairs) (cadr pairs)) pairs)))
...
(define (list-partitions total max-pieces max-value)
  (define (extend s x) 
        (cond
        ((null? s) x) 
        (else (cons (car s) (extend (cdr s) x)))
        )) 
  (define (filter lst)
       (if (null? lst) 0 (+ 1 (filter (cdr lst)))))
  ...
  (cond
    ...
    ((= max-value 0) nil)
    (else (begin 
      (define with_max (list-partitions (- total max-value) max-pieces max-value)) 
      (define without_max (list-partitions total max-pieces (- max-value 1)))
      (define (add_max lst) (cons max-value lst))
      
      (extend (keep-if (lambda (lst) (<= (filter lst) max-pieces)) (apply-to-all add_max with_max)) 
        (keep-if (lambda (lst) (<= (filter lst) max-pieces) )without_max)) 
      ))))
  ...
  (cond ((atom? expr)
        ...
        ((let? expr)
         (let ((values (cadr expr))
               (body   (cddr expr)))
           (cons (cons 'lambda (cons (apply-to-all analyze (car (zip values))) 
            (apply-to-all analyze body))) (apply-to-all analyze (cadr (zip values))))
           ))
        (else
