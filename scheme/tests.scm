...
(define g (mu () (lambda (y) (* x y z))))
(define (f x) (begin (define z (* x x)) (+ x ((g) z))))
(f 4)
; expect 1028

(define f (mu (x y) (begin
  (define z 2)
  (+ x y z))))
...
(begin (define (f x) (* 3 x)) (define (g y) (f y)) (g 3))
; expect 9

(begin '(4) '(3))
; expect (3)

(let ((x '(4)) (y (* 4 5))) (list x y))
; expect ((4) 20)

(if (> 4 5) (/ 1 0) '(4 5))
; expect (4 5)

(if (begin (define x 3) '5) x)
; expect 3

(if (begin (define x 3) '(5) (> x 5)) x 'wrong)
; expect wrong
...
(define (tally names)  
  (apply-to-all (lambda (x) (cons x (count x names))) (unique names) ))  
  ...
  (cond
    ...
    (else 
      (define (helper x new)
        (cond 
          ((null? new) nil)
          ((eq? x (car new)) (helper x (cdr new)))
          (else (cons (car new) (helper x (cdr new))))))
      (cons (car s) (unique (helper (car s) s)))
      )))
...
; expect ((jane . 5) (jack . 2) (jill . 1))

(define (deep-map fn s)
  (cond 
    ((null? s) nil)
    ((integer? (car s)) (cons (fn (car s)) (deep-map fn (cdr s))))
