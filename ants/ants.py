    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant.container:
                ...
                return None
            if hasattr(self.ant, "queens"):
                if self.ant.queens == 1:
                    return None
            self.ant = None
        else:
...
class Ant(Insect):
    ...
    def can_contain(self, other_ant):
        if self.container and self.ant==None and other_ant.container==False:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        bee_check = ThrowerAnt()
        bee_check.place = self.place.entrance
        bee_check.min_range = self.min_range
        bee_check.max_range = self.max_range
        bee_check.distance = self.distance
        if self.distance > self.max_range or self.place == hive:
            return None
        elif self.distance < self.min_range or self.place.bees == []:   
            bee_check.distance += 1 
            return bee_check.nearest_bee(hive)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            Place.remove_insect(place, self)
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        QueenAnt.queens += 1
        self.queens=QueenAnt.queens
        Ant.__init__(self)
        self.doubled_damage_ants = []
    def action(self, colony):
        ...
        if self.queens > 1:
            ...
            def count_ants(place, direction): #finds ants in queen ant's tunnel
                next_place = getattr(place, direction)
                if next_place == None:
                    return []
                else:
                    ant = next_place.ant
                    if ant and ant.container and ant.ant: #add the bodyguard and ant
                        return [ant] + [ant.ant] + count_ants(next_place, direction)
                    elif ant:
                        return [ant] + count_ants(next_place, direction)
                    return count_ants(next_place, direction)

            ants_in_tunnel = count_ants(self.place, "exit") + count_ants(self.place, "entrance") 
            if self.place.ant.container: #if queen a has bodyguard, add the bodyguard to the list
                ants_in_tunnel += [self.place.ant]    
            for ant in ants_in_tunnel:
                if ant not in self.doubled_damage_ants:
                    ant.damage *= 2
